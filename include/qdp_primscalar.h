  ...
  PScalar& operator=(const typename WordType<T>::Type_t& rhs)
    {
      ...
      return *this;
        ...
        template <class T1>
        inline PScalar &operator=(const PScalar<T1> &rhs)
        {
            elem() = rhs.elem();
            return *this;
        }
        ...
        //! Deep copies here
        PScalar(const PScalar &a) : F(a.F)
        { /* fprintf(stderr,"copy PScalar\n"); */
        }
    ...
    //! Ascii input
    template <class T>
    inline std::istream &operator>>(std::istream &s, PScalar<T> &d)
    {
        return s >> d.elem();
    }
    ...
    //! Ascii input
    template <class T>
    inline StandardInputStream &operator>>(StandardInputStream &s, PScalar<T> &d)
    {
        return s >> d.elem();
    }
    ...
    //! Ascii output
    template <class T>
    inline std::ostream &operator<<(std::ostream &s, const PScalar<T> &d)
    {
        return s << d.elem();
    }
    ...
    //! Ascii output
    template <class T>
    inline StandardOutputStream &operator<<(StandardOutputStream &s, const PScalar<T> &d)
    {
        return s << d.elem();
    }
    ...
    //! Text input
    template <class T>
    inline TextReader &operator>>(TextReader &txt, PScalar<T> &d)
    {
        return txt >> d.elem();
    }
    ...
    //! Text output
    template <class T>
    inline TextWriter &operator<<(TextWriter &txt, const PScalar<T> &d)
    {
        return txt << d.elem();
    }
    ...
    //! XML output
    template <class T>
    inline XMLWriter &operator<<(XMLWriter &xml, const PScalar<T> &d)
    {
        return xml << d.elem();
    }
    ...
    //! XML input
    template <class T>
    inline void read(XMLReader &xml, const std::string &path, PScalar<T> &d)
    {
...
//  typedef T1 Type_t;
    ...
    // PScalar = i * (1+lambda) * PScalar
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnTimesILambda>::Type_t
    timesILambda(const PScalar<T> &s1)
    {
        return timesILambda(s1.elem());
    }

    // PScalar = i * (1+lambda) * PScalar
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnTimesLambda>::Type_t
    timesLambda(const PScalar<T> &s1)
    {
        return timesLambda(s1.elem());
    }

    // PScalar = -i * (1+lambda) * PScalar
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnTimesMinusILambda>::Type_t
    timesMinusILambda(const PScalar<T> &s1)
    {
        return timesMinusILambda(s1.elem());
    }

    // PScalar = i *(1-lambda/3) * PScalar
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnTimesILambdaThird>::Type_t
    timesILambdaThird(const PScalar<T> &s1)
    {
        return timesILambdaThird(s1.elem());
    }

    // PScalar = i *(1-lambda/3) * PScalar
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnTimesLambdaThird>::Type_t
    timesLambdaThird(const PScalar<T> &s1)
    {
        return timesLambdaThird(s1.elem());
    }

    // PScalar = -i * (1-lambda/3) * PScalar
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnTimesMinusILambdaThird>::Type_t
    timesMinusILambdaThird(const PScalar<T> &s1)
    {
        return timesMinusILambdaThird(s1.elem());
    }
 ...
 */
    ...
    //! PScalar = SpinProject(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinProjectDir0MinusFull>::Type_t
    spinProjectDir0MinusFull(const PScalar<T> &s1)
    {
        return spinProjectDir0MinusFull(s1.elem());
    }

    //! PScalar = SpinReconstruct(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinReconstructDir0MinusFull>::Type_t
    spinReconstructDir0MinusFull(const PScalar<T> &s1)
    {
        return spinReconstructDir0MinusFull(s1.elem());
    }

    //! PScalar = SpinProject(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinProjectDir1MinusFull>::Type_t
    spinProjectDir1MinusFull(const PScalar<T> &s1)
    {
        return spinProjectDir1MinusFull(s1.elem());
    }

    //! PScalar = SpinReconstruct(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinReconstructDir1MinusFull>::Type_t
    spinReconstructDir1MinusFull(const PScalar<T> &s1)
    {
        return spinReconstructDir1MinusFull(s1.elem());
    }

    //! PScalar = SpinProject(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinProjectDir2MinusFull>::Type_t
    spinProjectDir2MinusFull(const PScalar<T> &s1)
    {
        return spinProjectDir2MinusFull(s1.elem());
    }

    //! PScalar = SpinReconstruct(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinReconstructDir2MinusFull>::Type_t
    spinReconstructDir2MinusFull(const PScalar<T> &s1)
    {
        return spinReconstructDir2MinusFull(s1.elem());
    }

    //! PScalar = SpinProject(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinProjectDir3MinusFull>::Type_t
    spinProjectDir3MinusFull(const PScalar<T> &s1)
    {
        return spinProjectDir3MinusFull(s1.elem());
    }

    //! PScalar = SpinReconstruct(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinReconstructDir3MinusFull>::Type_t
    spinReconstructDir3MinusFull(const PScalar<T> &s1)
    {
        return spinReconstructDir3MinusFull(s1.elem());
    }

    //! PScalar = SpinProject(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinProjectDir0PlusFull>::Type_t
    spinProjectDir0PlusFull(const PScalar<T> &s1)
    {
        return spinProjectDir0PlusFull(s1.elem());
    }

    //! PScalar = SpinReconstruct(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinReconstructDir0PlusFull>::Type_t
    spinReconstructDir0PlusFull(const PScalar<T> &s1)
    {
        return spinReconstructDir0PlusFull(s1.elem());
    }

    //! PScalar = SpinProject(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinProjectDir1PlusFull>::Type_t
    spinProjectDir1PlusFull(const PScalar<T> &s1)
    {
        return spinProjectDir1PlusFull(s1.elem());
    }

    //! PScalar = SpinReconstruct(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinReconstructDir1PlusFull>::Type_t
    spinReconstructDir1PlusFull(const PScalar<T> &s1)
    {
        return spinReconstructDir1PlusFull(s1.elem());
    }

    //! PScalar = SpinProject(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinProjectDir2PlusFull>::Type_t
    spinProjectDir2PlusFull(const PScalar<T> &s1)
    {
        return spinProjectDir2PlusFull(s1.elem());
    }

    //! PScalar = SpinReconstruct(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinReconstructDir2PlusFull>::Type_t
    spinReconstructDir2PlusFull(const PScalar<T> &s1)
    {
        return spinReconstructDir2PlusFull(s1.elem());
    }

    //! PScalar = SpinProject(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinProjectDir3PlusFull>::Type_t
    spinProjectDir3PlusFull(const PScalar<T> &s1)
    {
        return spinProjectDir3PlusFull(s1.elem());
    }

    //! PScalar = SpinReconstruct(PScalar)
    template <class T>
    inline typename UnaryReturn<PScalar<T>, FnSpinReconstructDir3PlusFull>::Type_t
    spinReconstructDir3PlusFull(const PScalar<T> &s1)
    {
        return spinReconstructDir3PlusFull(s1.elem());
    }
