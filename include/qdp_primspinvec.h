 ...
 */
    ...
    public:
        ...
        template <class T1>
        inline PSpinVector &assign(const PSpinVector<T1, N> &rhs)
        {
            for (int i = 0; i < N; i++)
                elem(i) = rhs.elem(i);
        ...
        template <class T1>
        inline PSpinVector &operator=(const PSpinVector<T1, N> &rhs)
        {
            return assign(rhs);
        }
        ...
        template <class T1>
        inline PSpinVector &operator*=(const PScalar<T1> &rhs)
        {
            for (int i = 0; i < N; ++i)
                elem(i) *= rhs.elem();
    ...
    //! Stream input
    template <class T, int N>
    inline std::istream &operator>>(std::istream &s, PSpinVector<T, N> &d)
    {
        for (int i = 0; i < N; ++i)
            s >> d.elem(i);
    ...
    template <class T, int N>
    inline StandardInputStream &operator>>(StandardInputStream &s, PSpinVector<T, N> &d)
    {
        for (int i = 0; i < N; ++i)
            s >> d.elem(i);
    ...
    //! Stream output
    template <class T, int N>
    inline std::ostream &operator<<(std::ostream &s, const PSpinVector<T, N> &d)
    {
        for (int i = 0; i < N; ++i)
            s << d.elem(i);
    ...
    //! Stream output
    template <class T, int N>
    inline StandardOutputStream &operator<<(StandardOutputStream &s, const PSpinVector<T, N> &d)
    {
        for (int i = 0; i < N; ++i)
            s << d.elem(i);
    ...
    //! Text input
    template <class T, int N>
    inline TextReader &operator>>(TextReader &txt, PSpinVector<T, N> &d)
    {
        for (int i = 0; i < N; ++i)
            txt >> d.elem(i);
    ...
    //! Text output
    template <class T, int N>
    inline TextWriter &operator<<(TextWriter &txt, const PSpinVector<T, N> &d)
    {
        for (int i = 0; i < N; ++i)
            txt << d.elem(i);
    ...
    //! XML output
    template <class T, int N>
    inline XMLWriter &operator<<(XMLWriter &xml, const PSpinVector<T, N> &d)
    {
    ...
    //! PSpinVector = i * (1+lambda) * PSpinVector
    template <class T, int N>
    inline typename UnaryReturn<PSpinVector<T, N>, FnTimesILambda>::Type_t
    timesILambda(const PSpinVector<T, N> &s1)
    {
        typename UnaryReturn<PSpinVector<T, N>, FnTimesILambda>::Type_t d;

        for (int i = 0; i < N; ++i)
            d.elem(i) = timesILambda(s1.elem(i));

        return d;
    }

    template <class T, int N>
    inline typename UnaryReturn<PSpinVector<T, N>, FnTimesLambda>::Type_t
    timesLambda(const PSpinVector<T, N> &s1)
    {
        typename UnaryReturn<PSpinVector<T, N>, FnTimesLambda>::Type_t d;

        for (int i = 0; i < N; ++i)
            d.elem(i) = timesLambda(s1.elem(i));
    ...
    //! PSpinVector = -i * (1+lambda) * PSpinVector
    template <class T, int N>
    inline typename UnaryReturn<PSpinVector<T, N>, FnTimesMinusILambda>::Type_t
    timesMinusILambda(const PSpinVector<T, N> &s1)
    {
        typename UnaryReturn<PSpinVector<T, N>, FnTimesMinusILambda>::Type_t d;

        for (int i = 0; i < N; ++i)
            d.elem(i) = timesMinusILambda(s1.elem(i));

        return d;
    }

    //! PSpinVector = i * (1-lambda/3) * PSpinVector
    template <class T, int N>
    inline typename UnaryReturn<PSpinVector<T, N>, FnTimesILambdaThird>::Type_t
    timesILambdaThird(const PSpinVector<T, N> &s1)
    {
        typename UnaryReturn<PSpinVector<T, N>, FnTimesILambdaThird>::Type_t d;

        for (int i = 0; i < N; ++i)
            d.elem(i) = timesILambdaThird(s1.elem(i));
    ...
    //! PSpinVector = i * (1-lambda/3) * PSpinVector
    template <class T, int N>
    inline typename UnaryReturn<PSpinVector<T, N>, FnTimesLambdaThird>::Type_t
    timesLambdaThird(const PSpinVector<T, N> &s1)
    {
        typename UnaryReturn<PSpinVector<T, N>, FnTimesLambdaThird>::Type_t d;

        for (int i = 0; i < N; ++i)
            d.elem(i) = timesLambdaThird(s1.elem(i));
    ...
    //! PSpinVector = -i * (1-lambda/3) * PSpinVector
    template <class T, int N>
    inline typename UnaryReturn<PSpinVector<T, N>, FnTimesMinusILambdaThird>::Type_t
    timesMinusILambdaThird(const PSpinVector<T, N> &s1)
    {
        typename UnaryReturn<PSpinVector<T, N>, FnTimesMinusILambdaThird>::Type_t d;

        for (int i = 0; i < N; ++i)
            d.elem(i) = timesMinusILambdaThird(s1.elem(i));
...
//  typedef T1 Type_t;
   ...
   */
    ...
    // spin projection for each direction
    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinProjectDir0PlusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinProjectDir0PlusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinProjectDir1PlusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinProjectDir1PlusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinProjectDir2PlusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinProjectDir2PlusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinProjectDir3PlusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinProjectDir3PlusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinProjectDir0MinusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinProjectDir0MinusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinProjectDir1MinusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinProjectDir1MinusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinProjectDir2MinusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinProjectDir2MinusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinProjectDir3MinusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinProjectDir3MinusFull>::Type_t, (N)> Type_t;
    };

    // spin reconstruction for each direction
    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinReconstructDir0PlusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinReconstructDir0PlusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinReconstructDir1PlusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinReconstructDir1PlusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinReconstructDir2PlusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinReconstructDir2PlusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinReconstructDir3PlusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinReconstructDir3PlusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinReconstructDir0MinusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinReconstructDir0MinusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinReconstructDir1MinusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinReconstructDir1MinusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinReconstructDir2MinusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinReconstructDir2MinusFull>::Type_t, (N)> Type_t;
    };

    template <class T, int N>
    struct UnaryReturn<PSpinVector<T, N>, FnSpinReconstructDir3MinusFull>
    {
        typedef PSpinVector<typename UnaryReturn<T, FnSpinReconstructDir3MinusFull>::Type_t, (N)> Type_t;
    };

    // SpinVector<4> = SpinProject(SpinVector<4>)
    // There are 4 cases here for Nd=4 for each forward/backward direction
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir0MinusFull>::Type_t
    spinProjectDir0MinusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir0MinusFull>::Type_t d;
   ...
   *      ( b3r + i b3i )     ( {a3r - a0i} + i{a3i + a0r} )     ( - b0i + i b0r ) 
   */
        d.elem(0) = s1.elem(0) - timesILambdaThird(s1.elem(3));
        d.elem(1) = s1.elem(1) - timesILambdaThird(s1.elem(2));
        d.elem(2) = s1.elem(2) + timesILambdaThird(s1.elem(1));
        d.elem(3) = s1.elem(3) + timesILambdaThird(s1.elem(0));

        return d;
    }

    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir1MinusFull>::Type_t
    spinProjectDir1MinusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir1MinusFull>::Type_t d;
   ...
   *      ( b3r + i b3i )     ( {a3r + a0r} + i{a3i + a0i} )     (   b0r + i b0i ) 
   */
        d.elem(0) = s1.elem(0) + timesLambdaThird(s1.elem(3));
        d.elem(1) = s1.elem(1) - timesLambdaThird(s1.elem(2));
        d.elem(2) = s1.elem(2) - timesLambdaThird(s1.elem(1));
        d.elem(3) = s1.elem(3) + timesLambdaThird(s1.elem(0));

        return d;
    }

    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir2MinusFull>::Type_t
    spinProjectDir2MinusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir2MinusFull>::Type_t d;
   ...
   *      ( b3r + i b3i )     ( {a3r + a1i} + i{a3i - a1r} )     (   b1i - i b1r )
   */
        d.elem(0) = s1.elem(0) - timesILambdaThird(s1.elem(2));
        d.elem(1) = s1.elem(1) + timesILambdaThird(s1.elem(3));
        d.elem(2) = s1.elem(2) + timesILambdaThird(s1.elem(0));
        d.elem(3) = s1.elem(3) - timesILambdaThird(s1.elem(1));

        return d;
    }

    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir3MinusFull>::Type_t
    spinProjectDir3MinusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir3MinusFull>::Type_t d;
   ...
   *      ( b3r + i b3i )     ( {a3r - a1r} + i{a3i - a1i} )     ( - b1r - i b1i ) 
   */
        d.elem(0) = s1.elem(0) - timesLambda(s1.elem(2));
        d.elem(1) = s1.elem(1) - timesLambda(s1.elem(3));
        d.elem(2) = s1.elem(2) - timesLambda(s1.elem(0));
        d.elem(3) = s1.elem(3) - timesLambda(s1.elem(1));

        return d;
    }

    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir0PlusFull>::Type_t
    spinProjectDir0PlusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir0PlusFull>::Type_t d;
   ...
   */
    ...
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir1PlusFull>::Type_t
    spinProjectDir1PlusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir1PlusFull>::Type_t d;
   ...
   */
    ...
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir2PlusFull>::Type_t
    spinProjectDir2PlusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir2PlusFull>::Type_t d;
   ...
   */
    ...
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir3PlusFull>::Type_t
    spinProjectDir3PlusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinProjectDir3PlusFull>::Type_t d;
   ...
   */
    ...
    // SpinVector<4> = SpinReconstruct(SpinVector<4>)
    // There are 4 cases here for Nd=4 for each forward/backward direction
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir0MinusFull>::Type_t
    spinReconstructDir0MinusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir0MinusFull>::Type_t d;
    ...
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir1MinusFull>::Type_t
    spinReconstructDir1MinusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir1MinusFull>::Type_t d;
    ...
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir2MinusFull>::Type_t
    spinReconstructDir2MinusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir2MinusFull>::Type_t d;
    ...
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir3MinusFull>::Type_t
    spinReconstructDir3MinusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir3MinusFull>::Type_t d;
    ...
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir0PlusFull>::Type_t
    spinReconstructDir0PlusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir0PlusFull>::Type_t d;
    ...
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir1PlusFull>::Type_t
    spinReconstructDir1PlusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir1PlusFull>::Type_t d;
    ...
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir2PlusFull>::Type_t
    spinReconstructDir2PlusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir2PlusFull>::Type_t d;
    ...
    template <class T>
    inline typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir3PlusFull>::Type_t
    spinReconstructDir3PlusFull(const PSpinVector<T, 4> &s1)
    {
        typename UnaryReturn<PSpinVector<T, 4>, FnSpinReconstructDir3PlusFull>::Type_t d;
