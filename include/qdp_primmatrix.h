 ...
 */
    ...
    public:
        ...
        template <class T1>
        inline CC &assign(const PScalar<T1> &rhs)
        {
            for (int i = 0; i < N; ++i)
                for (int j = 0; j < N; ++j)
                    if (i == j)
  ...
  PMatrix(const PMatrix& a)
    ...
    //! Text input
    template <class T, int N, template <class, int> class C>
    inline TextReader &operator>>(TextReader &txt, PMatrix<T, N, C> &d)
    {
        for (int j = 0; j < N; ++j)
            for (int i = 0; i < N; ++i)
                txt >> d.elem(i, j);
    ...
    //! Text output
    template <class T, int N, template <class, int> class C>
    inline TextWriter &operator<<(TextWriter &txt, const PMatrix<T, N, C> &d)
    {
        for (int j = 0; j < N; ++j)
            for (int i = 0; i < N; ++i)
                txt << d.elem(i, j);
    ...
    //! XML output
    template <class T, int N, template <class, int> class C>
    inline XMLWriter &operator<<(XMLWriter &xml, const PMatrix<T, N, C> &d)
    {
        xml.openTag("Matrix");
 ...
 *  and call transposeSpin on each one */
    ...
    //! PMatrix = i * (1+lambda) * PMatrix
    template <class T, int N, template <class, int> class C>
    struct UnaryReturn<PMatrix<T, N, C>, FnTimesILambda>
    {
        typedef C<typename UnaryReturn<T, FnTimesILambda>::Type_t, N> Type_t;
    };

    template <class T, int N, template <class, int> class C>
    inline typename UnaryReturn<PMatrix<T, N, C>, FnTimesILambda>::Type_t
    timesILambda(const PMatrix<T, N, C> &s1)
    {
        typename UnaryReturn<PMatrix<T, N, C>, FnTimesILambda>::Type_t d;

        for (int i = 0; i < N; ++i)
            for (int j = 0; j < N; ++j)
                d.elem(i, j) = timesILambda(s1.elem(i, j));

        return d;
    }

    //! PMatrix = i * (1-lambda/3) * PMatrix
    template <class T, int N, template <class, int> class C>
    struct UnaryReturn<PMatrix<T, N, C>, FnTimesILambdaThird>
    {
        typedef C<typename UnaryReturn<T, FnTimesILambdaThird>::Type_t, N> Type_t;
    };

    template <class T, int N, template <class, int> class C>
    inline typename UnaryReturn<PMatrix<T, N, C>, FnTimesILambdaThird>::Type_t
    timesILambdaThird(const PMatrix<T, N, C> &s1)
    {
        typename UnaryReturn<PMatrix<T, N, C>, FnTimesILambdaThird>::Type_t d;

        for (int i = 0; i < N; ++i)
            for (int j = 0; j < N; ++j)
                d.elem(i, j) = timesILambdaThird(s1.elem(i, j));

        return d;
    }

    //! PMatrix = (1+lambda) * PMatrix
    template <class T, int N, template <class, int> class C>
    struct UnaryReturn<PMatrix<T, N, C>, FnTimesLambda>
    {
        typedef C<typename UnaryReturn<T, FnTimesLambda>::Type_t, N> Type_t;
    };

    template <class T, int N, template <class, int> class C>
    inline typename UnaryReturn<PMatrix<T, N, C>, FnTimesLambda>::Type_t
    timesLambda(const PMatrix<T, N, C> &s1)
    {
        typename UnaryReturn<PMatrix<T, N, C>, FnTimesLambda>::Type_t d;

        for (int i = 0; i < N; ++i)
            for (int j = 0; j < N; ++j)
                d.elem(i, j) = timesLambda(s1.elem(i, j));

        return d;
    }

    //! PMatrix = (1-lambda/3) * PMatrix
    template <class T, int N, template <class, int> class C>
    struct UnaryReturn<PMatrix<T, N, C>, FnTimesLambdaThird>
    {
        typedef C<typename UnaryReturn<T, FnTimesLambdaThird>::Type_t, N> Type_t;
    };

    template <class T, int N, template <class, int> class C>
    inline typename UnaryReturn<PMatrix<T, N, C>, FnTimesLambdaThird>::Type_t
    timesLambdaThird(const PMatrix<T, N, C> &s1)
    {
        typename UnaryReturn<PMatrix<T, N, C>, FnTimesLambdaThird>::Type_t d;

        for (int i = 0; i < N; ++i)
            for (int j = 0; j < N; ++j)
                d.elem(i, j) = timesLambdaThird(s1.elem(i, j));
