...
struct ordered_fused_spin_proj_user_arg_full
{
    const OLattice<SU3Mat> &u;
    const OLattice<FVec> &a;
    OLattice<FVec> &d;
...
inline void ordered_fused_spin_proj_evaluate_function(int lo, int hi, int myId, ordered_fused_spin_proj_user_arg *arg)
{
...
inline void ordered_fused_spin_proj_evaluate_function_full(int lo, int hi, int myId, ordered_fused_spin_proj_user_arg_full *arg)
{
    ...
    for (int site = low; site < high; site++)
    {
        ...
        _inline_mult_adj_su3_mat_vec(u.elem(site).elem(), tmp.elem(0), d.elem(site).elem(0));
        _inline_mult_adj_su3_mat_vec(u.elem(site).elem(), tmp.elem(1), d.elem(site).elem(1));
        _inline_mult_adj_su3_mat_vec(u.elem(site).elem(), tmp.elem(2), d.elem(site).elem(2));
        _inline_mult_adj_su3_mat_vec(u.elem(site).elem(), tmp.elem(3), d.elem(site).elem(3));
    }
}
...
inline void unordered_fused_spin_proj_evaluate_function(int lo, int hi, int myId, unordered_fused_spin_proj_user_arg *arg)
{
...
struct unordered_fused_spin_proj_user_arg_full
{
    const OLattice<SU3Mat> &u;
    const OLattice<FVec> &a;
    OLattice<FVec> &d;
...
// user func for evaluate NOT having order
inline void unordered_fused_spin_proj_evaluate_function_full(int lo, int hi, int myId, unordered_fused_spin_proj_user_arg_full *arg)
{
