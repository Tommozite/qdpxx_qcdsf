...
#include "qdp_scalarsite_generic_linalg_wrapper.h"
    ...
    template <>
    inline void evaluate(OLattice<PSpinVector<PColorVector<RComplexFloat, 3>, 2>> &d,
                         const OpAssign &op,
    ...
    template <>
    inline void evaluate(OLattice<PSpinVector<PColorVector<RComplexFloat, 3>, 4>> &d,
                         const OpAssign &op,
                         const QDPExpr<BinaryNode<OpMultiply,
                                                  Reference<QDPType<PScalar<PColorMatrix<RComplexFloat, 3>>,
                                                                    OLattice<PScalar<PColorMatrix<RComplexFloat, 3>>>>>,
                                                  Reference<QDPType<PSpinVector<PColorVector<RComplexFloat, 3>, 4>,
                                                                    OLattice<PSpinVector<PColorVector<RComplexFloat, 3>, 4>>>>>,
                                       OLattice<PSpinVector<PColorVector<RComplexFloat, 3>, 4>>> &rhs,
                         const Subset &s)
    {
#if defined(QDP_SCALARSITE_DEBUG)
        cout << "specialized QDP_H_M_times_H" << endl;
        ...
        const C &l = static_cast<const C &>(rhs.expression().left());
        const F &r = static_cast<const F &>(rhs.expression().right());
        ...
        if (s.hasOrderedRep())
        {
            ...
            ordered_linalg_user_arg_full a(d, l, r, base);

            dispatch_to_threads(totalSize, a, ordered_linalg_evaluate_userfunc_full);
        ...
        else
        {
            ...
            unordered_linalg_user_arg_full arg(d, l, r, tab);

            dispatch_to_threads(totalSize, arg, unordered_linalg_evaluate_userfunc_full);
