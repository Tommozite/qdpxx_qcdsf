...
typedef OLattice<PSpinVector<PColorVector<RComplexFloat, 3>, 4>> F;
...
struct ordered_linalg_user_arg_full
{
    OLattice<PSpinVector<PColorVector<RComplexFloat, 3>, 4>> &d;
    const C &l;
    const F &r;
    int base;
    ordered_linalg_user_arg_full(
        OLattice<PSpinVector<PColorVector<RComplexFloat, 3>, 4>> &d_,
        const C &l_,
        const F &r_,
        int base_) : d(d_), l(l_), r(r_), base(base_) {}
};
...
//
inline void ordered_linalg_evaluate_userfunc(int lo, int hi, int myId, ordered_linalg_user_arg *a)
{
...
inline void ordered_linalg_evaluate_userfunc_full(int lo, int hi, int myId, ordered_linalg_user_arg_full *a)
{

    OLattice<PSpinVector<PColorVector<RComplexFloat, 3>, 4>> &d = a->d;
    const C &l = a->l;
    const F &r = a->r;
    int base = a->base;
...
struct unordered_linalg_user_arg_full
{
    OLattice<PSpinVector<PColorVector<RComplexFloat, 3>, 4>> &d;
    const C &l;
    const F &r;
    ...
    unordered_linalg_user_arg_full(
        OLattice<PSpinVector<PColorVector<RComplexFloat, 3>, 4>> &d_,
        const C &l_,
        const F &r_,
        const int *tab_) : d(d_), l(l_), r(r_), tab(tab_) {}
...
//
//template<>
inline void unordered_linalg_evaluate_userfunc(int lo, int hi, int myId, unordered_linalg_user_arg *a)
{
...
inline void unordered_linalg_evaluate_userfunc_full(int lo, int hi, int myId, unordered_linalg_user_arg_full *a)
{
